...
use crate::tab_configs::tab_config::{
    copy_env_files_to_worktree_command, generated_worktree_repo_dir, TabConfigPaneType,
};
...
fn test_materialized_default_worktree_config_renders_full_worktree_path() {
    ...
    if let PaneTemplateType::PaneTemplate { commands, .. } = pane_template {
        ...
        assert_eq!(
            commands[1].exec,
            copy_env_files_to_worktree_command(&expected_worktree_path)
        );
        assert_eq!(commands[2].exec, format!("cd {expected_worktree_path}"));
    } else {
...
#[cfg(feature = "local_fs")]
#[test]
fn test_materialized_existing_worktree_config_inserts_env_copy() {
    let template = r#"
name = "Worktree"
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{autogenerated_branch_name}} {{worktree_path_prefix}}{{autogenerated_branch_name}}",
  "cd {{worktree_path_prefix}}{{autogenerated_branch_name}}",
]
"#;
    let repo_path = "/tmp/example-repo";
    let (_, tab_config) =
        materialize_default_worktree_config(template, "Worktree: example-repo", repo_path, "agent")
            .expect("expected existing template materialization to succeed");
